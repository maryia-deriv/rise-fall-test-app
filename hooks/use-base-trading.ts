...
import { useDerivWS, useActiveSymbols, useTicks } from '@deriv/core';
 ...
 *
 * Handles WebSocket lifecycle, active-symbol discovery, tick streaming,
 * open/closed position tracking, and contract selling. Templates compose
 * this hook and layer their own proposal-param computation and buy logic on top.
 */
export function useBaseTrading({
  ...
  const { ws, isConnected, isExhausted, error } = useDerivWS(wsUrl);

  // When the authenticated WS exhausts all reconnect attempts, fall back to
  // the public WS by triggering logout (clears wsUrl → useDerivWS reconnects public).
  useEffect(() => {
    if (isExhausted && wsUrl) {
      onAuthWSFailed?.();
    }
  }, [isExhausted, wsUrl, onAuthWSFailed]);
